...
with open('json/5esrd.json', encoding="utf8") as json_file:
...
class Jeffbot():
    def __init__(self, client):
        ...
        with open('users.json') as userfile:
            ...
            for user in json_users.keys():
                ...
                self.users[user].display_name = json_users[user]["display_name"]
                self.users[user].characters = json_users[user]["characters"]
            userfile.close()

    def get_rulebook(self, section):
        message = ""
        if section[2] == "classes":
            with open('json/classes.json', encoding="utf8") as json_file:
                ...
                for key in data.keys():
                    all_classes += str(key) + " "
                return "The classes are:\n" + str(all_classes)

        elif section[2] == "races":
            with open('json/races.json', encoding="utf8") as json_file:
                data = json.load(json_file)
                try:
                    for race in data["Races"]:
                        if race == section[3]:
                            if race[0] == section[4]:  # traits
                                return race[0]["content"]
                except:
            ...
            return "Failed to parse rulebook command, this function is still under construction"

    def roll(self, parsed_message):
        try:
            numbers_rolled = ""
            numbers = parsed_message[2].split("d")
            ...
            numbers_rolled = "You rolled: " + numbers_rolled
            return numbers_rolled

        except:
            return "Invalid format. Second word must consist of the letter d followed by the "\
    ...
    #                         "Send two numbers as the minimum and maximum bounds for a random number generator, for example:"
    #                         "\n !jeff 1 100")
    #
    #                 if msgList[1] == "lie":
    ...
    #                     returnMessage = "Your random number is " + str(
    #                         ran) + " as divined by an incredibly smart and handsome programmer."
    ...
    def parse_message(self, chat_message):
        # print("[+] '{}' from group ID {} says: {}".format(chat_message.from_jid, chat_message.group_jid,
        #                                                   chat_message.body))
        # if self.should_ignore:
        #     return
        if chat_message.lower() == "deez":
            return "nuts"

        parsed_message = chat_message.lower().split()


        if chat_message.startswith('!jeff'):
            if len(parsed_message) == 2:
                if parsed_message[1] == "clear":
                    self.rerollNumbers.clear()
                    return "The list of rerolls has been cleared!"

                if parsed_message[1] == "owo":
                    return "https://www.youtube.com/watch?v=h6DNdop6pD8"
                ...
                if parsed_message[1] == "lie":
                    return ("I am pizi's totally real and not suspicious boyfriend")

                elif parsed_message[1] == "commands":
                    return ("Jeffbot's regular commands are: \n"
                            "   !jeff help\n"
                            "   !jeff <number1> <number2>\n"
                            "Jeffbot's DnD commands are: \n"
                            "   !dnd rulebook\n"
                            "   !dnd roll\n")
                            # "   select character\n"
                            # "   create character\n")

                elif parsed_message[1] == "rulebook":
                    return self.get_rulebook(parsed_message)
                    # "-send noods")
                elif parsed_message[1] == "roll":
                    return self.roll(parsed_message)

                else:
                    try:
                        print(self.rerollNumbers)
                        ran = random.randint(0, int(parsed_message[1]))
                        # setFlag = False
                        # for person in self.rerollNumbers:
                        #     if ran != person:
                        #         setFlag = True
                        #         break
                        #
                        # if not setFlag:
                        #     self.rerollNumbers.clear()
                        #     return "The entire set has been exhausted, clearing the list now!"
                        ...
                        for person in self.rerollNumbers:
                            if ran == person:
                                # ran = random.randint(0, int(parsed_message[1]))
                                return "A number was rerolled too many times... Try again!"

                        self.rerollNumbers.append(ran)
                        returnMessage = "Your random number is " + str(
                            ran) + " as divined by an incredibly smart and handsome programmer."
            ...
            if len(parsed_message) == 3:
                try:
                    ...
                    setFlag = False
                    rangeList = [*range(int(parsed_message[1]), int(parsed_message[2])+1)]
                    print(rangeList)
                        # if ran != person:
                        #     setFlag = True
                        #     break

                    if set(rangeList) == set(self.rerollNumbers):
                        self.rerollNumbers.clear()
                        return "The entire set has been exhausted, clearing the list now!"

                    print(self.rerollNumbers)
                    ran = random.randint(int(parsed_message[1]), int(parsed_message[2]))
                    returnMessage += "Your random number is " + str(ran)

                    for person in self.rerollNumbers:
                        if ran == person:
                            returnMessage += ".\n Yahhh, it's REROLL TIME! "
                            ran = random.randint(int(parsed_message[1]), int(parsed_message[2]))
                            returnMessage += "Your random number is " + str(ran)
                            break
                    ...
                    for person in self.rerollNumbers:
                        if ran == person:
                            # ran = random.randint(0, int(parsed_message[1]))
                            returnMessage += ".\nA number was rerolled too many times... Try again!"
                            return returnMessage

                    self.rerollNumbers.append(ran)
                    return (returnMessage + " as divined by an incredibly smart and handsome programmer.")
                except:
                    return (
                        "Send two numbers as the minimum and maximum bounds for a random number generator, for example:"
                        "\n !jeff 1 100")

        elif chat_message.startswith('!dnd'):
            if parsed_message[1] == "rulebook":
                return self.get_rulebook(parsed_message)
                # "-send noods")
            elif parsed_message[1] == "roll":
                return self.roll(parsed_message)
        ...
        # else:
            # try:
                ...
                #     return ("Jeffbot's DnD commands are: \n"
                #               "-current\n"
                #               "-roll\n"
                #               "-select character\n"
                #               "-create character")
